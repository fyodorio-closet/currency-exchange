...
let AppComponent = class AppComponent {
    constructor(calculator, store, fb) {
    ...
    ngOnInit() {
        ...
        this.subscription.add(this.currencyForm.controls['sourceCurrency'].valueChanges.subscribe(change => {
            ...
            this.targetCurrencyList = this.targetCurrencyList.filter(item => change !== item);
        ...
        this.subscription.add(this.currencyForm.controls['requestDate'].valueChanges.subscribe(change => {
            ...
            if (newValue && newValue !== oldValue)
                this.store.dispatch(new _state_rates_actions__WEBPACK_IMPORTED_MODULE_4__["LoadRatesByDate"](Object.assign({}, this.dataSet, { date: newValue, base: this.currentSourceCurrencyName })));
        }));
    }
    ngOnDestroy() {
...
let AppService = class AppService {
    getRatesByAnotherBase(rates, newBase) {
...
let DataFetcherService = class DataFetcherService {
    ...
    getDataByDate(action) {
        const date = action.payload.date;
        const base = action.payload.base;
        return this.http.get(`${API_URL_BASE}${date}`).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["map"])((rateSet) => this.calculator.getRatesByAnotherBase(rateSet, base)));
    }
};
...
class LoadRatesByDate {
    constructor(payload) {
        this.payload = payload;
        this.type = RatesActionTypes.LoadRatesByDate;
    }
}
LoadRatesByDate.ctorParameters = () => [
    { type: undefined }
];
class RatesLoaded {
    constructor(payload) {
...
/* harmony import */ var _rates_actions__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./rates.actions */ "./src/app/state/rates.actions.ts");
...
function ratesReducer(state = initialRatesState, action) {
    switch (action.type) {
        case _rates_actions__WEBPACK_IMPORTED_MODULE_0__["RatesActionTypes"].RatesLoaded: {
            return Object.assign({}, state, { ratesSet: action.payload });
        }
        case _rates_actions__WEBPACK_IMPORTED_MODULE_0__["RatesActionTypes"].LoadRatesByDate: {
            return Object.assign({}, state, { ratesSet: Object.assign({}, state.ratesSet, { date: action.payload.date }) });
        }
